...
The installer can run while Codex or the Dream Skin tray is active. When Codex is detected, it asks for confirmation before closing, updating, and relaunching it; the tray is stopped and restored automatically. Normal use does not require administrator access or ownership changes under WindowsApps.
...
The installer validates the official Codex Store package and Node.js, saves a recoverable appearance baseline, and initializes the local theme store. It is safe to run repeatedly, and an active tray does not need to be closed manually. By default it also creates two shortcuts:

- `Codex Dream Skin`: launch or reapply the skin and ensure the system tray theme controls are running.
...
The `Codex Dream Skin` shortcut is the recommended launcher. It asks for confirmation before restarting an open Codex window and automatically opens the tray controls after the skin starts successfully.
...
After launching `Codex Dream Skin`, right-click its notification-area icon to:
...
### Codex is running during installation

Confirm the installer's prompt to let it close Codex, stop the current tray and recorded injector, perform the update, then relaunch the tray and Codex. Choosing No cancels normally without changing the configuration. If installation fails, the installer attempts to restore the tray and reopen stock Codex.
