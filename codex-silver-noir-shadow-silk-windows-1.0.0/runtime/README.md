...
安装脚本支持在 Codex 或 Dream Skin 托盘正在运行时执行。检测到 Codex 后会先请求确认，再安全关闭、更新并重新启动；托盘会自动停止和恢复。普通使用不需要管理员权限，也不需要接管 WindowsApps 目录。
...
安装器会校验官方 Codex Store 包和 Node.js，保存可恢复的外观配置，并初始化本地主题仓库。安装器可以重复运行，活动托盘无需手动退出。默认还会创建两个快捷方式：

- `Codex Dream Skin`：启动或重新应用皮肤，并自动确保系统托盘主题控制正在运行。
...
推荐从 `Codex Dream Skin` 快捷方式启动。它发现 Codex 已经运行时会先询问是否重启，并在皮肤成功启动后自动打开托盘控制。
...
从 `Codex Dream Skin` 启动后，右键任务栏通知区域中的托盘图标，可以：
...
确认安装器弹出的提示后，它会关闭 Codex、停止当前托盘和已记录的注入器，完成更新后重新启动托盘与 Codex。选择“否”会正常取消，不修改配置。如果安装失败，安装器会尽力恢复托盘并重新打开普通 Codex。
