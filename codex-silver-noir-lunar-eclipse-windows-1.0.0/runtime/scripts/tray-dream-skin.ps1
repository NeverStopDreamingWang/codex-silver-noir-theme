  ...
  function Rebuild-DreamSkinTrayMenu {
    ...
    if ($savedThemes.Count -eq 0) {
      ...
      foreach ($saved in $savedThemes) {
        ...
        $savedAction = {
          $applied = Use-DreamSkinSavedTheme -ThemeDirectory $savedPath -StateRoot $StateRoot
          Set-DreamSkinPaused -Paused $false -StateRoot $StateRoot | Out-Null
          if ($applied.NativeAppearanceChanged) {
            $notify.ShowBalloonTip(2200, 'Codex Dream Skin', "已应用：$savedName；原生外观需要重启 Codex。", [System.Windows.Forms.ToolTipIcon]::Info)
            Start-DreamSkinPowerShell -Script $startScript -Arguments @('-Port', "$Port", '-PromptRestart')
          } else {
