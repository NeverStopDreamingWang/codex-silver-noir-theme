...
$script:DreamSkinNativeAppearanceKeys = @(
  'appearanceTheme',
  'appearanceDarkCodeThemeId',
  'appearanceDarkChromeTheme'
)

function Assert-DreamSkinNativeAppearance {
  param([AllowNull()][object]$NativeAppearance)
  if ($null -eq $NativeAppearance) { return }
  if ($NativeAppearance -is [string] -or $NativeAppearance -is [array]) {
    throw 'nativeAppearance must be an object.'
  }
  if ("$($NativeAppearance.variant)" -cne 'dark') {
    throw 'nativeAppearance.variant must be dark.'
  }
  $codeThemeId = "$($NativeAppearance.codeThemeId)"
  if (-not $codeThemeId -or $codeThemeId.Length -gt 120 -or $codeThemeId -match '[\u0000-\u001f]') {
    throw 'nativeAppearance.codeThemeId must be a short single-line string.'
  }
  $chrome = $NativeAppearance.chromeTheme
  if ($null -eq $chrome -or $chrome -is [string] -or $chrome -is [array]) {
    throw 'nativeAppearance.chromeTheme must be an object.'
  }
  foreach ($property in @('accent', 'ink', 'surface')) {
    if ("$($chrome.$property)" -notmatch '^#[0-9A-Fa-f]{6}$') {
      throw "nativeAppearance.chromeTheme.$property must be a #RRGGBB color."
    }
  }
  $contrast = 0
  if (-not [int]::TryParse("$($chrome.contrast)", [ref]$contrast) -or $contrast -lt 0 -or $contrast -gt 100) {
    throw 'nativeAppearance.chromeTheme.contrast must be an integer from 0 through 100.'
  }
  if ($chrome.opaqueWindows -isnot [bool]) {
    throw 'nativeAppearance.chromeTheme.opaqueWindows must be a boolean.'
  }
  $fonts = $chrome.fonts
  if ($null -eq $fonts -or $fonts -is [string] -or $fonts -is [array]) {
    throw 'nativeAppearance.chromeTheme.fonts must be an object.'
  }
  foreach ($property in @('code', 'ui')) {
    $font = "$($fonts.$property)"
    if (-not $font -or $font.Length -gt 120 -or $font -match '[\u0000-\u001f]') {
      throw "nativeAppearance.chromeTheme.fonts.$property must be a short single-line string."
    }
  }
  $semantic = $chrome.semanticColors
  if ($null -eq $semantic -or $semantic -is [string] -or $semantic -is [array]) {
    throw 'nativeAppearance.chromeTheme.semanticColors must be an object.'
  }
  foreach ($property in @('diffAdded', 'diffRemoved', 'skill')) {
    if ("$($semantic.$property)" -notmatch '^#[0-9A-Fa-f]{6}$') {
      throw "nativeAppearance.chromeTheme.semanticColors.$property must be a #RRGGBB color."
    }
  }
}

function ConvertTo-DreamSkinTomlString {
  param([Parameter(Mandatory = $true)][AllowEmptyString()][string]$Value)
  return '"' + $Value.Replace('\', '\\').Replace('"', '\"') + '"'
}

function Get-DreamSkinNativeAppearanceLines {
  param([Parameter(Mandatory = $true)][object]$NativeAppearance)
  Assert-DreamSkinNativeAppearance -NativeAppearance $NativeAppearance
  $chrome = $NativeAppearance.chromeTheme
  $fonts = $chrome.fonts
  $semantic = $chrome.semanticColors
  $opaque = if ([bool]$chrome.opaqueWindows) { 'true' } else { 'false' }
  $chromeLine = 'appearanceDarkChromeTheme = { accent = ' +
    (ConvertTo-DreamSkinTomlString "$($chrome.accent)") + ', contrast = ' + [int]$chrome.contrast +
    ', fonts = { code = ' + (ConvertTo-DreamSkinTomlString "$($fonts.code)") +
    ', ui = ' + (ConvertTo-DreamSkinTomlString "$($fonts.ui)") + ' }, ink = ' +
    (ConvertTo-DreamSkinTomlString "$($chrome.ink)") + ', opaqueWindows = ' + $opaque +
    ', semanticColors = { diffAdded = ' + (ConvertTo-DreamSkinTomlString "$($semantic.diffAdded)") +
    ', diffRemoved = ' + (ConvertTo-DreamSkinTomlString "$($semantic.diffRemoved)") +
    ', skill = ' + (ConvertTo-DreamSkinTomlString "$($semantic.skill)") + ' }, surface = ' +
    (ConvertTo-DreamSkinTomlString "$($chrome.surface)") + ' }'
  return [ordered]@{
    appearanceTheme = 'appearanceTheme = "dark"'
    appearanceDarkCodeThemeId = 'appearanceDarkCodeThemeId = ' +
      (ConvertTo-DreamSkinTomlString "$($NativeAppearance.codeThemeId)")
    appearanceDarkChromeTheme = $chromeLine
  }
}

function Get-DreamSkinNativeAppearanceMarkerPath {
  param([string]$StateRoot = (Join-Path $env:LOCALAPPDATA 'CodexDreamSkin'))
  return Join-Path ([System.IO.Path]::GetFullPath($StateRoot)) 'native-appearance.json'
}

function Read-DreamSkinNativeAppearanceMarker {
  param([string]$StateRoot = (Join-Path $env:LOCALAPPDATA 'CodexDreamSkin'))
  $path = Get-DreamSkinNativeAppearanceMarkerPath -StateRoot $StateRoot
  if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { return $null }
  Assert-DreamSkinNoReparseComponents -Path $path
  try { $marker = (Read-DreamSkinUtf8File -Path $path) | ConvertFrom-Json -ErrorAction Stop } catch {
    throw "Native appearance restore marker is unreadable; Codex config was preserved: $path"
  }
  if ($null -eq $marker -or [int]$marker.schemaVersion -ne 1 -or
    $null -eq $marker.original -or $null -eq $marker.managed) {
    throw "Native appearance restore marker is invalid; Codex config was preserved: $path"
  }
  return $marker
}

function Set-DreamSkinNativeAppearance {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][string]$ConfigPath,
    [Parameter(Mandatory = $true)][object]$NativeAppearance,
  ...
  if (Test-DreamSkinDesktopNestedTable -Content $content -Key 'appearanceDarkCodeThemeId') {
    throw "Refusing to replace 'appearanceDarkCodeThemeId' because it is represented as a nested desktop table."
  }
  $hasNestedDarkChromeTheme = Test-DreamSkinDesktopNestedTable `
    -Content $content -Key 'appearanceDarkChromeTheme'
  ...
  $lines = Get-DreamSkinNativeAppearanceLines -NativeAppearance $NativeAppearance
  $marker = Read-DreamSkinNativeAppearanceMarker -StateRoot $StateRoot
  if ($null -eq $marker) {
    $original = [ordered]@{}
    foreach ($key in $script:DreamSkinNativeAppearanceKeys) {
      $original[$key] = Get-DreamSkinSectionSettingLine -Body $desktop.Body -Key $key
    }
  } else {
    $original = [ordered]@{}
    foreach ($key in $script:DreamSkinNativeAppearanceKeys) { $original[$key] = $marker.original.$key }
  }
  $body = $desktop.Body
  foreach ($key in $script:DreamSkinNativeAppearanceKeys) {
    if ($key -eq 'appearanceDarkChromeTheme' -and $hasNestedDarkChromeTheme) { continue }
    $body = Set-DreamSkinSectionSetting -Body $body -Key $key -Line $lines[$key] -NewLine $newLine
  }
  $updated = $content.Substring(0, $desktop.BodyStart) + $body +
    $content.Substring($desktop.BodyStart + $desktop.BodyLength)
  $managed = [ordered]@{}
  foreach ($key in $script:DreamSkinNativeAppearanceKeys) {
    $managed[$key] = if ($key -eq 'appearanceDarkChromeTheme' -and $hasNestedDarkChromeTheme) {
      $null
    } else {
      $lines[$key]
    }
  }
  $markerContent = [ordered]@{ schemaVersion = 1; original = $original; managed = $managed } |
    ConvertTo-Json -Depth 5
  $markerPath = Get-DreamSkinNativeAppearanceMarkerPath -StateRoot $StateRoot
  Assert-DreamSkinNoReparseComponents -Path $markerPath
  Write-DreamSkinUtf8FileAtomically -Path $markerPath -Content ($markerContent + "`r`n")
  Write-DreamSkinUtf8FileAtomically -Path $ConfigPath -Content $updated -ExpectedBytes $originalBytes
  return ($updated -cne $content)
}

function Restore-DreamSkinNativeAppearance {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][string]$ConfigPath,
    [string]$StateRoot = (Join-Path $env:LOCALAPPDATA 'CodexDreamSkin')
  ...
  $body = $desktop.Body
  foreach ($key in $script:DreamSkinNativeAppearanceKeys) {
    $current = Get-DreamSkinSectionSettingLine -Body $body -Key $key
    if ($current -ceq "$($marker.managed.$key)") {
      $body = Set-DreamSkinSectionSetting -Body $body -Key $key -Line $marker.original.$key -NewLine $newLine
    }
  }
  $updated = $content.Substring(0, $desktop.BodyStart) + $body +
    $content.Substring($desktop.BodyStart + $desktop.BodyLength)
...
function Read-DreamSkinTheme {
  ...
  Assert-DreamSkinNativeAppearance -NativeAppearance $theme.nativeAppearance
  $image = "$($theme.image)"
...
function Use-DreamSkinSavedTheme {
  ...
  $active = Set-DreamSkinActiveTheme -ImagePath $saved.ImagePath -Theme $theme -StateRoot $StateRoot
  $nativeAppearanceChanged = if ($null -ne $theme.nativeAppearance) {
    Set-DreamSkinNativeAppearance -ConfigPath $ConfigPath -NativeAppearance $theme.nativeAppearance `
      -StateRoot $StateRoot
  } else {
    Restore-DreamSkinNativeAppearance -ConfigPath $ConfigPath -StateRoot $StateRoot
  }
  $active | Add-Member -NotePropertyName NativeAppearanceChanged `
    -NotePropertyValue ([bool]$nativeAppearanceChanged) -Force
  return $active
}
